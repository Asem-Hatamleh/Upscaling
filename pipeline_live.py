...
Architecture:
    [Capture thread (user-supplied stream reader)]
...
Stream reader: drop in your own. Expected interface: an instance exposing
a `.frame_queue` (thread-safe `queue.Queue` of BGR np.ndarray) plus an
awaitable `.run()` coroutine. Provide via `--reader-module module:Class`.
...
def _load_reader(spec: str):
    """Load 'module:ClassName' as the stream reader.
    Class must expose .frame_queue (queue.Queue of BGR ndarrays) and async .run()."""
    if ":" not in spec:
        raise ValueError(f"--reader-module must be 'module:Class', got {spec!r}")
    mod_name, cls_name = spec.split(":", 1)
    mod = importlib.import_module(mod_name)
    return getattr(mod, cls_name)

# Optional: upscaler from sibling repo Upscaling/
_UPSCALE_ROOT = Path(__file__).parent / "Upscaling"
if _UPSCALE_ROOT.exists() and str(_UPSCALE_ROOT) not in sys.path:
    sys.path.insert(0, str(_UPSCALE_ROOT))
...
def capture_thread(player, capture_q: queue.Queue, stop_event: threading.Event, max_seq: int | None):
    """Run reader in its own event loop; push frames w/ timestamps."""
...
def main() -> int:
    ...
    # (stream-source flags removed; pass reader via --reader-module + --reader-arg)
    # pipeline
    ...
    ap.add_argument("--show-raw", action="store_true", help="also show raw reader window")
    ap.add_argument("--reader-module", required=True,
                    help="stream reader as 'module:Class'. Class needs .frame_queue (queue.Queue of BGR ndarray) and async .run(). "
                         "Extra kwargs forwarded via --reader-arg key=value (repeatable).")
    ap.add_argument("--reader-arg", action="append", default=[],
                    help="key=value pairs forwarded to reader constructor. Repeatable.")
    ...
    # build reader from user-supplied module
    ReaderCls = _load_reader(args.reader_module)
    reader_kwargs = {}
    for kv in args.reader_arg:
        if "=" not in kv:
            raise ValueError(f"--reader-arg must be key=value, got {kv!r}")
        k, v = kv.split("=", 1)
        # try cast common types
        if v.lower() in ("true", "false"):
            reader_kwargs[k] = v.lower() == "true"
        else:
            try:
                reader_kwargs[k] = int(v)
            except ValueError:
                try:
                    reader_kwargs[k] = float(v)
                except ValueError:
                    reader_kwargs[k] = v
    player = ReaderCls(**reader_kwargs)
    if not hasattr(player, "frame_queue") or not hasattr(player, "run"):
        raise RuntimeError(f"{args.reader_module} must expose .frame_queue and .run()")
